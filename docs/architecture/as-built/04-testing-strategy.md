...
- 테스트 함수 수(정적 집계, `def test_`/`async def test_` 기준, parametrize 전개 전): `test_apidocs` 14, `test_client` 16, `test_config` 10, `test_daemon` 4, `test_errors` 4, `test_host_guard` 4, `test_integration` 2, `test_jobs` 16, `test_pool` 14, `test_server` 15, `test_winjob` 3, `test_worker` 9, `fixtures/test_fake_claude_cli` 4 — 합계 115.
...
| 서브프로세스 단위 | `Worker`, `WorkerPool`, `JobStore` | 가짜 CLI를 실제 자식 프로세스로 스폰 | `tests/test_worker.py`, `tests/test_pool.py`, `tests/test_jobs.py:42-146` |
| HTTP 인프로세스 | 라우트·미들웨어 | `pytest-aiohttp`의 `aiohttp_client(create_app(pool))` | `tests/test_server.py:24-29`, `tests/test_jobs.py:149-232`, `tests/test_apidocs.py:25-29` |
...
| OS 통합(Windows 한정) | Job Object | `pytestmark = skipif(sys.platform != "win32")`, `test_pool.py:260` 개별 skipif | `tests/test_winjob.py`, `tests/test_pool.py:260-295` |
...
- 축소 루프 검증은 `pool._idle`·`pool._total`에 만료된 `Worker`를 직접 넣는다(`tests/test_integration.py:79-84`, `tests/test_pool.py:123-140`).
- 일부 테스트는 짧은 `asyncio.sleep`과 작은 `idle_timeout_sec`/`scale_down_interval_sec`에 기댄다(예: `tests/test_pool.py:22-23`, `tests/test_integration.py:65`, `tests/test_integration.py:86`).
- 풀 teardown은 `conftest.make_pool` fixture가 `stop()`한다(`tests/conftest.py:16-29`). `tests/test_pool.py:227-257`의 두 테스트는 `WorkerPool`을 직접 만들고 본문에서 `stop()`한다. Windows 전용 테스트(`tests/test_pool.py:260-295`)도 직접 만들며 `stop()` 대신 `pool._job.Close()`와 워커 `kill()`로 정리한다.
...
- 확인한 소스: `tests/**/*.py` 17개 전부, [pyproject.toml](../../../pyproject.toml), [.gitignore](../../../.gitignore), `scripts/*.py` 3개
- 확인 범위: 테스트 본문·fixture 정적 읽기와 함수 수 정적 집계. **테스트를 실행하지 않았으므로 통과 여부·소요 시간·커버리지는 기록하지 않는다.**
