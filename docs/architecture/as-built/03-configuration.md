...
- 값의 출처는 두 가지다: 데이터클래스 기본값(`config.py:23-44`)과 환경변수를 읽는 `PoolConfig.from_env()`(`config.py:66-109`).
...
| 필드 | 환경변수 | 폴백값 | 변환 | 사용처 |
|---|---|---|---|---|
| `min_workers` | `CLAUDE_POOL_MIN_WORKERS` | 4 | `int()` | 예열·초과분 축소 하한 (`pool.py:47`, `pool.py:199`, `pool.py:223`). 유휴 축소가 발동하면 이 하한은 무시된다 |
| `max_workers` | `CLAUDE_POOL_MAX_WORKERS` | 30 | `int()` | 증가 상한 (`pool.py:154`, `pool.py:177`, `pool.py:200`) |
...
| `acquire_timeout_sec` | `CLAUDE_POOL_ACQUIRE_TIMEOUT_SEC` | 60.0 | `float()` | 워커 획득 대기 상한 (`pool.py:129`) |
| `idle_timeout_sec` | `CLAUDE_POOL_IDLE_TIMEOUT_SEC` | 60.0 | `float()` | 초과분 축소 대상 판정 (`pool.py:229`) |
| `scale_down_interval_sec` | `CLAUDE_POOL_SCALE_DOWN_INTERVAL_SEC` | 30.0 | `float()` | 축소 루프 주기 (`pool.py:208`) |
| `idle_scale_to_zero_sec` | `CLAUDE_POOL_IDLE_SCALE_TO_ZERO_SEC` | 60.0 | `float()` | 유휴 축소(0까지) 판정. 마지막 반납 이후 경과가 이 값을 넘고 배포된 워커가 없으면 idle 전부 종료. `0`이면 판정이 꺼져 `min_workers` 하한이 유지된다 (`config.py:32-36`, `config.py:100-102`, `pool.py:216-224`) |
...
- 환경변수 표의 근거: `config.py:68-109`. 데이터클래스 기본값도 같은 값이다(`config.py:23-44`).
...
`PoolConfig.__post_init__`(`config.py:46-64`)가 직접 생성과 `from_env()` 두 경로 모두에서 실행된다.

| 규칙 | 위치 | 실패 시 |
|---|---|---|
| `host`가 `"localhost"`이거나 `ipaddress.ip_address(host).is_loopback`이어야 함 (`is_loopback_host`, `config.py:12-18`) | `config.py:49-54` | `ValueError`(메시지에 `loopback`) |
| `claude_cmd`가 비어 있지 않은 list | `config.py:57-60` | `ValueError`(메시지에 `claude_cmd`) |
| `claude_cmd` 원소가 모두 str | `config.py:61-64` | `ValueError` |
| `CLAUDE_POOL_CLAUDE_CMD_JSON` JSON 파싱 | `config.py:68-75` | `ValueError("CLAUDE_POOL_CLAUDE_CMD_JSON is not valid JSON: ...")` |

- 숫자 환경변수의 `int()`/`float()` 변환 실패는 별도 처리 없이 `ValueError`로 전파된다(`config.py:87-106`). `idle_scale_to_zero_sec`의 음수·범위 검사는 없다(`0` 이하이면 유휴 축소 판정이 항상 거짓이 된다, `pool.py:217`).
...
- 갱신한 절: 「설정 소스」·「환경변수와 코드 폴백값」·「검증」. `idle_scale_to_zero_sec` 행을 추가하고 `config.py`·`pool.py` 줄 번호를 이번 tree 기준으로 다시 읽었다.
- 확인한 소스: [config.py](../../../src/claude_pool/config.py)(전체), [pool.py](../../../src/claude_pool/pool.py)(`acquire`·`release`·`_scale_down_loop` 경로), [test_config.py](../../../tests/test_config.py)
- 확인 범위: 설정 정의·검증·사용 지점 정적 확인. 환경변수를 실제로 주입해 실행하지 않았다.

### 이전 기준 — `821f6e9c83edc2b2f11434cc00e52c106f6f7b42`

- 위에 적지 않은 나머지 절(「주입 경로」·「시크릿」·「pytest 설정」·「빌드 설정」)과 `config.py`·`pool.py` 외 파일의 줄 번호는 이 commit 기준이며 이번에 다시 확인하지 않았다.
- 당시 확인한 소스: [daemon.py](../../../src/claude_pool/daemon.py), [server.py](../../../src/claude_pool/server.py), [worker.py](../../../src/claude_pool/worker.py), [jobs.py](../../../src/claude_pool/jobs.py), [client.py](../../../src/claude_pool/client.py), [apidocs.py](../../../src/claude_pool/apidocs.py), [daemon_ctl.py](../../../scripts/daemon_ctl.py), [pyproject.toml](../../../pyproject.toml)
