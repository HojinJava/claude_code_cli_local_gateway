---
description: claude-pool 저장소의 실제 폴더·진입점 구성과 분석 대상 37개 파일의 기능별 책임표(File Responsibility Map)
tags: [architecture]
status: as-built
---
...
- `scripts/`는 패키지 탐색 대상(`src`) 밖이다. `scripts/daemon_ctl.py`는 `claude_pool`을 import한다(`scripts/daemon_ctl.py:17-19`). `scripts/ask.py`도 `claude_pool.client`를 import한다(`scripts/ask.py:14`).
...
| 줄바꿈 정규화 | [`.gitattributes`](../../../.gitattributes) | 단일 규칙 `* text=auto eol=lf` — 추적 대상 전 파일을 저장소에서 LF로 정규화 |
...
| 설정 (데몬·클라이언트 운영 공용) | [`src/claude_pool/config.py`](../../../src/claude_pool/config.py) | `PoolConfig` 데이터클래스와 기본값(유휴 축소 기준 시간 `idle_scale_to_zero_sec` 포함, `config.py:32-36`), `from_env()` 환경변수 로딩(`CLAUDE_POOL_IDLE_SCALE_TO_ZERO_SEC` 포함, `config.py:100-102`), `__post_init__`의 loopback host·`claude_cmd` 검증, Host 가드와 공유하는 `is_loopback_host` 판정 |
...
| 워커 풀 | [`src/claude_pool/pool.py`](../../../src/claude_pool/pool.py) | `WorkerPool`: 풀당 kill-on-close job 생성, min 예열, acquire 대기·수요 기반 증가, 1회용 release(kill 후 보충, 마지막 반납 시각 `_last_release_at` 기록 `pool.py:198`), idle 축소 루프(초과분 축소와 유휴 시 idle 전부 종료 판정 `pool.py:206-240`), stop 드레인, 마지막 오류 기록, `/health`용 `stats()` |
...
| 프롬프트 1건 런처 | [`scripts/ask.py`](../../../scripts/ask.py) | 인자를 공백으로 이어 프롬프트 1건을 만들고 `ClaudePoolClient().generate()`로 전송(클라이언트 자동 기동 경로 사용), 결과를 stdout에 출력. 빈 프롬프트는 usage 출력 후 종료 코드 2, `ClaudePoolError`는 `kind`·`retryable`·`retry_after_sec`를 stderr에 출력 후 종료 코드 1 |
...
| 설정 | [`tests/test_config.py`](../../../tests/test_config.py) | 기본값, 환경변수 override, loopback host 허용·거부, `scratch_dir`·`claude_cmd`(JSON) 검증, `idle_scale_to_zero_sec` 기본값·환경변수·`0` 값 |
...
| 워커 풀 | [`tests/test_pool.py`](../../../tests/test_pool.py) | 예열, acquire/release·보충, max 한도, 축소 루프, 유휴 시 0까지 축소·축소 후 재요청·배포된 워커가 있을 때 비축소·기준 시간 `0`의 하한 유지, 취소된 run의 kill, 죽은 idle 폐기, 스폰 실패, stop 드레인·대기자 깨우기, Windows job 핸들 닫힘 시 워커 종료 |
...
| 백그라운드 job | [`tests/test_jobs.py`](../../../tests/test_jobs.py) | `JobStore` 제출·실패 기록·취소·종료·reap·상한, 실행 중 job이 유휴 축소를 막는지, `/jobs` 라우트 202·폴링·404·검증·목록·health job 수·Host 가드 |
...
| 통합 | [`tests/test_integration.py`](../../../tests/test_integration.py) | 동시 요청 간 응답 섞임 없음, 버스트 증가와 idle 축소, 0까지 축소된 풀의 `/health`·`/generate` 응답과 반납 후 1개 재예열 |
| 런처 | [`tests/test_ask_script.py`](../../../tests/test_ask_script.py) | `scripts/ask.py`를 `importlib`로 로드하고 `ClaudePoolClient`를 stub 클래스로 monkeypatch해 성공 출력·종료 코드 0, `ClaudePoolError`의 `kind`·`retryable`·`retry_after_sec` stderr 출력·종료 코드 1, 빈 인자 usage·종료 코드 2, docstring 실행 예시 존재를 확인 |

### 분석 제외

| 경로 | 사유 |
|---|---|
| `README.md`, `CLAUDE.md`, `AGENTS.md` | 사용자·에이전트 문서. `pyproject.toml`에 `readme` 키가 없고 빌드·코드가 참조하지 않는다. 명령 출처로만 인용한다 |
| `.`으로 시작하는 폴더(`.git/`, 로컬 `.pytest_cache/` 등) | 추적 대상 아님 또는 도구 산출물 |
| `docs/` | spec·plan·아키텍처 문서. 코드가 참조하지 않는다 |
| `__pycache__/`·`*.pyc` 등 | 자동 생성물(`.gitignore` 대상, 추적되지 않음) |

## 실측 근거

절마다 기준 시점이 다르다. 이번 갱신(이슈 #2)에서 다시 확인한 범위와, 그 전 시점 기준으로 남아 있는 범위를 구분해 적는다.

### 이번 갱신 기준 — `7cc905fc828abeb790f3e8d46de469e7021ed137`

- 갱신한 절: 「실제 폴더 구성」(`.gitattributes` 추가), 「주요 진입점」(`scripts/ask.py`), 「File Responsibility Map」의 대상 수(34 → 37)와 `.gitattributes`·`scripts/ask.py`·`tests/test_ask_script.py` 행, `config.py`·`pool.py`·`tests/test_config.py`·`tests/test_pool.py`·`tests/test_jobs.py`·`tests/test_integration.py` 행의 책임 서술, 「분석 제외」의 `AGENTS.md`·`docs/`.
- 파일 집합: `git ls-tree -r --name-only 7cc905fc828abeb790f3e8d46de469e7021ed137`에서 `README.md`·`CLAUDE.md`·`AGENTS.md`·`docs/**`를 뺀 37개 경로와 위 표의 경로 집합을 스크립트로 대조했다(일치).
- 확인한 소스: [`.gitattributes`](../../../.gitattributes), [`scripts/ask.py`](../../../scripts/ask.py), [`src/claude_pool/config.py`](../../../src/claude_pool/config.py), [`src/claude_pool/pool.py`](../../../src/claude_pool/pool.py), [`tests/test_ask_script.py`](../../../tests/test_ask_script.py), [`tests/test_config.py`](../../../tests/test_config.py), [`tests/test_pool.py`](../../../tests/test_pool.py), [`tests/test_jobs.py`](../../../tests/test_jobs.py), [`tests/test_integration.py`](../../../tests/test_integration.py) 본문.
- 확인 범위: 정적 읽기와 `git` 읽기 명령. 테스트·데몬·`claude` CLI를 실행하지 않았다.

### 이전 기준 — `821f6e9c83edc2b2f11434cc00e52c106f6f7b42`

- 위에 적지 않은 나머지 행·절은 이 commit 기준으로 확인한 내용이며 이번에 다시 확인하지 않았다. 당시 파일 집합 대조는 `README.md`·`CLAUDE.md`를 뺀 34개 경로 기준이었다.
- 당시 확인한 소스: 그 34개 파일 본문 전체(빈 `__init__.py` 3개는 줄 수 0 확인).
- 미확인: 없음(대상 파일 전량 확인). 단 위 나머지 행의 내용이 `7cc905fc…`에서도 동일한지는 이번에 파일 집합 대조 외에 재확인하지 않았다.
