...
- `tests/test_pool.py`: 0까지 축소, 처리 중 비축소, 기준 시간 `0`일 때 기존 동작 유지, 축소 후 재획득 시 온디맨드 스폰과 보충 1개를 확인한다. 대기 시간을 줄이려면 워커의 `became_idle_at`과 풀의 마지막 종료 시각을 직접 조작한다(기존 테스트가 쓰는 방식).
