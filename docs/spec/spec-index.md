...
세 spec은 모두 `origin: migration`이다. plan이 없으며, 변경하려면 `brain`에서 요구를 다시 구체화해야 make-plans의 계획 대상이 된다. 관련 구조 문서는 [architecture.md](../architecture/architecture.md)에서 찾는다.
