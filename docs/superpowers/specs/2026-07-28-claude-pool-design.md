...
[Python 호출자 A]  [Python 호출자 B]  [Python 호출자 C]  ...
   ...
   (claude -p, stdin 대기중)                  (풀 크기 = 설정 가능, 기본 16)
```
...
2. 데몬이 풀에서 idle 워커 하나를 꺼낸다 (없으면 idle 워커가 생길 때까지 대기 —
   v1은 큐잉만 하고 오버플로우 콜드 스폰은 하지 않는다).
...
GET /health
  200: { "pool_size": int, "idle": int, "busy": int }
```
...
- **풀 고갈 시**: idle 워커가 생길 때까지 요청을 큐잉한다 (v1은 오버플로우 콜드
  스폰 없음 — 벤치마크에서 큐잉 지연이 문제로 확인되면 추가를 검토한다).
