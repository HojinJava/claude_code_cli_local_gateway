...
- **데몬이 죽어도 워커는 고아로 안 남음**: 모든 워커는 Windows Job Object(`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`)에 묶여 있습니다. 정상 종료(`Ctrl+C`, 취소)는 `WorkerPool.stop()`이 처리하고, 강제 종료·크래시처럼 정리 코드가 전혀 실행되지 않는 경우에도 OS가 job 핸들이 닫히는 즉시 워커 프로세스를 전부 종료시킵니다.
